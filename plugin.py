...
class LastScannedAnalyzerScreen(Screen):
    ...
    def get_orbital_position(self, ref_string):
        parts = ref_string.split(':')
        if len(parts) >= 7:
            try:
                s_ns = int(parts[6], 16)
                ...
                if pos > 1800:
                    pos = 3600 - pos
                    return "%s°W" % (str(pos / 10.0))
                else:
                    return "%s°E" % (str(pos / 10.0))
            except ValueError:
                pass
        return "Unknown Sat"
    ...
    def apply_filter_and_sort(self):
        ...
        # 2. Filter
        filtered_channels = []
        ukupno = len(self.master_channel_data)
        novi = sum(1 for ch in self.master_channel_data if ch['is_new'])
        
        for ch in self.master_channel_data:
            # show only new filter
            if self.show_only_new and not ch['is_new']:
                continue
            # search name filter
            if self.search_filter:
                if self.search_filter.lower() not in ch['name'].lower():
                    continue
            filtered_channels.append(ch)
            
        # 3. Sort
        if self.sort_mode == "name":
            filtered_channels.sort(key=lambda x: x['name'].lower())
        elif self.sort_mode == "sat":
            # Group by satellite position
            def get_sat_num(ch):
                sat = ch['sat']
                try:
                    num_part = sat.split('°')[0]
                    num = float(num_part)
                    if 'W' in sat:
                        return -num
                    return num
                    ...
                    return 999
            filtered_channels.sort(key=get_sat_num)
        elif self.sort_mode == "fta":
            # Free To Air (FTA) first (not encrypted)
            filtered_channels.sort(key=lambda x: x['encrypted'])
        elif self.sort_mode == "res":
            # Resolution order: 4K (0x1f, 31), HD (0x19, 25, 0x11, 17), SD (others)
            def get_res_order(ch):
                c_type = ch['c_type']
                if c_type in [0x1f, 31]:
                    return 0 # 4K first
                elif c_type in [0x19, 25, 0x11, 17]:
                    return 1 # HD second
                return 2 # SD last
            filtered_channels.sort(key=get_res_order)
            
        # 4. Generate display lists
        for ch in filtered_channels:
            ...
            if self.sort_mode == "sat":
                display_text = "%s%s%s (%s)" % (sel_tag, tags, name, ch['sat'])
            else:
        ...
        # Update headers
        header_text = "TOTAL: %d  |  NEW: %d" % (ukupno, novi)
        if self.search_filter:
            header_text += "  |  [Filter: %s]" % self.search_filter
        if self.sort_mode != "default":
            sort_names = {"name": "A-Z", "sat": "Satelit", "fta": "FTA", "res": "Rezolucija"}
            header_text += "  |  [Sort: %s]" % sort_names.get(self.sort_mode, self.sort_mode)
            
        self.list[0] = header_text
        self.list[1] = "-" * 80
    ...
    def load_channels(self):
        ...
        if not os.path.exists(last_scanned_file):
            self.list = ["GRESKA: userbouquet.LastScanned.tv nije pronadjen!"]
            ...
            with io.open(last_scanned_file, 'r', encoding='utf-8', errors='ignore') as f:
                for line in f:
                    if line.startswith("#SERVICE"):
                        ...
                        self.master_channel_data.append({
                            'ref': ref,
                            'name': name,
                            'provider': provider,
                            'is_new': is_new,
                            ...
                            'apid': apid,
                            'encrypted': encrypted,
                            'sat': self.get_orbital_position(ref)
                        })
        except Exception as e:
            self.list = ["Error reading LastScanned.tv: " + str(e)]
            self.channel_data = [None]
            self["list"].setList(self.list)
            return
    ...
    def open_scan(self):
        """Open the main options menu."""
        options = [
            ("Signal Finder", "scan"),
            ("Pretraga / Filter po nazivu", "search"),
        ]
        if self.search_filter:
            options.append(("Ukloni pretragu / filter", "clear_search"))
            
        options.extend([
            ("Sortiranje kanala", "sort"),
            ("Grupna selekcija (Select / Deselect)", "group_select"),
            ...
            if action == "scan":
                ...
                if not opened:
                    try:
                        ...
                        self.session.openWithCallback(self.scan_finished, Satfinder)
                        opened = True
                    except ImportError:
                        pass
                if not opened:
                    try:
                        from Plugins.SystemPlugins.SignalFinder.plugin import SignalFinderMultistreamT2MI
                        self.session.openWithCallback(self.scan_finished, SignalFinderMultistreamT2MI)
                        opened = True
                    except ImportError:
                        pass
                if not opened:
                    try:
                        from Screens.SignalFinder import SignalFinder
                        self.session.openWithCallback(self.scan_finished, SignalFinder)
                        opened = True
                    except ImportError:
                        pass
                if not opened:
                    try:
                        ...
                        pass
                if not opened:
                    self.session.open(MessageBox, "Greška: Ne mogu da učitam Signal Finder modul.", MessageBox.TYPE_ERROR)
            elif action == "search":
                self.session.openWithCallback(self.search_callback, VirtualKeyBoard, title="Unesite pojam za pretragu:", text=self.search_filter or "")
            elif action == "clear_search":
                self.search_filter = None
                self.apply_filter_and_sort()
            elif action == "sort":
                sort_options = [
                    ("Podrazumevano (redosled skeniranja)", "default"),
                    ("Po imenu (A-Z)", "name"),
                    ("Po satelitu (orbitalna pozicija)", "sat"),
                    ("FTA / Kodirani (Slobodni na vrh)", "fta"),
                    ("Po rezoluciji (4K -> HD -> SD)", "res"),
                ]
                self.session.openWithCallback(self.sort_callback, ChoiceBox, title="Izaberite sortiranje:", list=sort_options)
            elif action == "group_select":
                select_options = [
                    ("Obeleži SVE kanale [ * ]", "select_all"),
                    ("Obeleži samo NOVE kanale [ * ]", "select_new"),
                    ("Odbeleži SVE kanale", "deselect_all"),
                ]
                self.session.openWithCallback(self.group_select_callback, ChoiceBox, title="Grupna selekcija:", list=select_options)
            elif action == "update":
                self.check_for_update(auto_check=False)

    def search_callback(self, text):
        if text is not None:
            self.search_filter = text.strip() if text.strip() else None
            self.apply_filter_and_sort()

    def sort_callback(self, choice):
        if choice:
            self.sort_mode = choice[1]
            self.apply_filter_and_sort()

    def group_select_callback(self, choice):
        if choice:
            action = choice[1]
            if action == "select_all":
                for ch in self.channel_data:
                    if ch:
                        self.selected_refs.add(ch[0])
            elif action == "select_new":
                for ch in self.channel_data:
                    if ch and ch[3]: # is_new
                        self.selected_refs.add(ch[0])
